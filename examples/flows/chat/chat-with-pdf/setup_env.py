...
BASE_DIR = f"{os.path.dirname(os.path.abspath(__file__))}/chat_with_pdf/"
...
def setup_env(connection: Union[AzureOpenAIConnection, OpenAIConnection], config: dict):
    ...
    with acquire_lock(f"{BASE_DIR}create_folder.lock"):
        if not os.path.exists(f"{BASE_DIR}.pdfs"):
            os.mkdir(f"{BASE_DIR}.pdfs")
        if not os.path.exists(f"{BASE_DIR}.index/.pdfs"):
            os.makedirs(f"{BASE_DIR}.index/.pdfs")
